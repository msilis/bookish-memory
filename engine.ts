...
export async function captureScreenshot(url: string, recipe: SiteRecipe) {
  const browser: Browser = await chromium.launch({ headless: true });
  const context = await browser.newContext({
    viewport: recipe.viewport || { width: 1280, height: 800 },
    userAgent:
      recipe.userAgent || "Mozilla/5.0 (Windows NT 10.0; Win64; x64) ...",
  });
  const page: Page = await context.newPage();
  try {
    await page.goto(url, {
      waitUntil: (recipe.waitCondition as any) || "networkidle",
      timeout: 60000,
    });
